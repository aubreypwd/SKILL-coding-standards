...
- Follow the **Breathing Room Principle** for function-like code: add a blank line after a named function or method opening brace when its body contains multiple logical statements or sections, or when its first statement is complex or multiline. Keep a trivial one-statement body compact, such as a function that only registers one hook; adding a blank line there would invent a separation that the code does not have. Apply the same spacing to an anonymous function, callback, or closure when its body contains multiple statements or logical sections; a one-statement callback may remain compact.
...
- When reading an optional or untrusted PHP array key, use null coalescing before comparing it. Choose the missing-value default according to the intended semantics: use `( $array['key'] ?? false )` when missing should behave as false or should fail a strict string comparison, and use `( $array['key'] ?? '' )` when missing should behave as an empty string. Parenthesize the coalesced expression because `??` has low precedence.
...
- Keep a nested component's own declarations together at the top of its block. Place root-level responsive overrides that change the component itself beside those declarations, before nested child concepts; do not make a reader scroll past child blocks to find the parent component's properties. Then organize states, pseudo-selectors, nested parts, and media rules according to the element or concept they modify.
- Place each media-query override with the selector it changes. A root component's media query contains only root-component changes; a child concept such as `.card__hero` gets its own nested media query. Do not create one component-wide media wrapper that mixes root and child overrides.
...
- Treat an HTML comment as its own logical child block. When a comment is inside an HTML parent or a PHP-controlled HTML region, leave a blank line after the parent opening tag before the comment and another blank line between the comment and the child it explains. Do not attach the comment directly to either boundary. A comment at the document or fragment root has no containing parent and does not need invented surrounding markup.
...
- Do not add comments that merely restate a function name, DocBlock, selector, property assignment, obvious call, or straightforward control flow. Do not use code comments to teach or justify a coding principle such as DRY, Inline Temp, ternary selection, or breathing room. The code should demonstrate the principle; the comment should describe domain behavior, non-obvious intent, a constraint, an edge case, accessibility or state behavior, or a meaningful structure. Do not hyper-document ordinary function calls. Summarize a call's purpose only when its role in the current function is not obvious, and keep the comment immediately before the code it describes. Keep comments accurate when the code changes.
...
- HTML comments count as logical child blocks and have breathing room from both the parent opening tag and the child they explain.
- The Breathing Room Principle was applied to named functions and methods with multiple logical statements or sections, and to functions whose first statement is complex or multiline. Trivial one-statement bodies remain compact. Nested callbacks and closures with multiple statements or logical sections have breathing room after their opening brace.
...
- Nested media queries are colocated with the selector they modify and do not mix root and child overrides in one wrapper.
...
- Optional PHP array keys are protected with `??` before comparison, with a deliberate default matching the intended missing-value semantics and explicit parentheses around the coalesced expression.
...
- Comments do not explain coding principles or justify obvious assignments; they convey reader-facing domain intent or non-obvious behavior.
